...
//数据库
/*
  DROP TABLE IF EXISTS `mz_auth_rule`;
  CREATE TABLE `mz_auth_rule` (
  `id` mediumint(8) unsigned NOT NULL AUTO_INCREMENT COMMENT '主键',
  `name` char(80) NOT NULL DEFAULT '' COMMENT '规则唯一标识',
  `title` char(20) NOT NULL DEFAULT '' COMMENT '规则中文名称',
  `type` tinyint(1) NOT NULL DEFAULT '1' COMMENT '如果type为1， condition字段就可以定义规则表达式。 如定义{score}>5  and {score}<100  表示用户的分数在5-100之间时这条规则才会通过',
  ...
  ) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='用户-组 映射表';
 */
...
class Auth
{
	 ...
	 * @param int          $type     如果type为1， condition字段就可以定义规则表达式。 如定义{score}>5  and {score}<100  表示用户的分数在5-100之间时这条规则才会通过。
	...
	protected function getUserInfo($uid)
	{
		...
		{
			$userInfo[$uid] = Db::name($this->config['auth_user'])
				->where(['uid' => $uid])
				->find();
		}
