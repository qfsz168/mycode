...
class ExifToolBatch
{
	...
	private $_exiftool = TOOL_EXIFTOOL.'exiftool';

	private $_defexecargs           = array('-use MWG');
	private $_defargs               = array(
		'-g',
		'-j',
		'-coordFormat',
		'%.6f',
	...
	/**
	 * ExifToolBatch constructor.
	 * @param string $path Exiftool path
	 * @param array  $args Default exec args
	 * @throws Exception
	 * @return ExifToolBatch $this
	 */
			...
			case "fgets": // fast, low cpu (blocking), med cpu (non-blocking)
				...
				else
				{
					...
					while (1)
					{
						...
						$str    = fgets($this->_pipes[$pipe], self::BUFF_SIZE);
						$output = $output.$str;
						if (substr($output, $endstr_len) == $endstr)
						{
							break;
						}
	...
	{
		if (is_array($this->_lasterr) && is_null($id))
		{
			return false;
		}
	...
	/**
	 *
	 * Fetch one item from stack and decode to one array
	 *
	 * @return mixed Data/False
	 */
	public function fetchDecodedOne()
	{
		if (!$this->fetch())
		{
			return false;
		...
		$data = json_decode($this->_lastdata, true);
		if (!$data)
		{
			return false;
		}
		...
		foreach ($data[0] as $key => $value)
		{
			if (is_object($value))
			{
				$arr = array_merge($arr, json_decode(json_encode($value), true));
			}
