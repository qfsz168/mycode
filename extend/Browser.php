...
class Browser
{
     ...
     * self define
     */
    public static function getInternetIp()
    {
        $ip         = null;
        $serverIp   = null;
        ...
        try
        {
            ...
            // 如果获取失败，返回null
            if(!isset($ip) || !isset($serverIp))
                return null;

            // 判断获取的Ip与服务器Ip是否是同网段
            $IpArr = explode('.', $ip);
            $serverIpArr = explode('.', $serverIp);

            // 如果前两个相同，认为是同网段如：192.168。返回null
            if($IpArr[0] == $serverIpArr[0] && $IpArr[1] == $serverIpArr[1])
                return null;
    ...
    {
        if (stripos($this->_agent, 'windows') !== false || stripos($this->_agent, 'win') !== false) {
        ...
        }elseif (stripos($this->_agent, 'linux')       !== false || stripos($this->_agent, 'FreeBSD') !== false ||
                 stripos($this->_agent, 'OpenBSD')     !== false || stripos($this->_agent, 'NetBSD')  !== false ||
                 stripos($this->_agent, 'OpenSolaris') !== false || stripos($this->_agent, 'SunOS')   !== false ||
                 stripos($this->_agent, 'OS\/2')       !== false || stripos($this->_agent, 'BeOS')    !== false) {
