...
class Ico
{
	 ...
	 * @return string
	 */
	public function Img2Ico(string $pngFile, string $newFile = null)
	{
		if (!is_file($pngFile))
		{
			return "";
		...
		$file = ROOT_PATH."temp".DIRECTORY_SEPARATOR."tmp.png";
		...
		$resize_im = @imagecreatetruecolor($imginfo[0], $imginfo[1]);

		imagecopyresampled($resize_im, $im, 0, 0, 0, 0, $imginfo[0], $imginfo[1], $imginfo[0], $imginfo[1]);
	 ...
	 * @return string
	 */
	protected function GD2ICOstring(&$gd_image_array)
	{
		$icANDmask = $ImageWidths = $ImageHeights = $totalcolors = $BitmapInfoHeader = $bpp = $icXOR = $icAND = [];
		...
		foreach ($gd_image_array as $key => $gd_image)
		{
			...
			$icondata .= $this->LittleEndian2String($dwImageOffset, 4);// dwImageOffset;
	 ...
	 * @return string
	 */
	protected function LittleEndian2String($number, $minbytes = 1)
	{
		$intstring = '';
		while ($number > 0)
		{
	 ...
	 * @return array|bool
	 */
	protected function GetPixelColor(&$img, $x, $y)
	{
		if (!is_resource($img))
		{
			return false;
